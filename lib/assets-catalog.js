...
var ObsidianAssetsCatalog = Class.$extend({
    ...
    importAssetCatalog: function(catalog) {
        for (var packName in catalog.packages) {
            ...
            this.$data.packList[packName].url = url.resolve(this.$data.rootUrl, pack.url);
    ...
    loadAsset: function(id) {
        ...
        if (!source) {
            return this.$data.manager.addAssetFromUrl(asset.url, {
                id: id,
                mime: asset.mime,
        ...
        // It's from a pack
            ...
            return this.$data.manager.importAssetPackageFromUrl(pack.url)
                .then(function(pack) {
