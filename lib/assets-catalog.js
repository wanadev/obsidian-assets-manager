...
var ObsidianAssetsCatalog = Class.$extend({
    ...
    importAssetCatalogFromUrl: function(catalogUrl) {
        var _this = this;
        return httpRequest.getJsonProxy(url.resolve(this.$data.rootUrl, catalogUrl))
    ...
    /**
     * [PROMISE] Ensure the pack is loaded in the asset manager.
     *
     * @method loadPack
     * @param {String} name
     * @return {String}
    ...
    loadPack: function(name) {
      var pack = this.$data.packList[name];

      if (!pack) {
          throw Error("Pack '" + name + "' does not exist");
      }

      if (!pack.loaded) {
        pack.loaded = this.$data.manager.importAssetPackageFromUrl(url.resolve(this.$data.rootUrl, pack.url))
            .then(function(pack) {
                if (pack.packName !== name) {
                    throw new Error("Pack name '" + name + "' in catalog is different from the one in the package itself '" + pack.packName + "'");
                }
