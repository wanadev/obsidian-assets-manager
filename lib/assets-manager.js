...
var BufferConverter = require("./buffer-converter");
...
var AssetsManager = Class.$extend({
    ...
    addAssetFromImage: function(image, options) {
        var _this = this;
        options = options || {};
        options.mime = (options.mime === "image/jpeg") ? "image/jpeg" : "image/png";

        if (!(image instanceof Image)) {
            throw new Error("NotAImage");
        }

        return BufferConverter.fromImage(image, options.mime)
            .then(function(buffer) {
                return _this.addAssetFromBuffer(buffer.data, options);
            })
            .then(function(id) {
    ...
    addAssetFromUrl: function(url, options) {
        ...
        return BufferConverter.fromUrl(url)
            .then(function(buffer) {
                options.mime = options.mime || buffer.mime;
                return _this.addAssetFromBuffer(buffer.data, options);
            });
    },
    ...
    addAssetFromData64Url: function(data64Url, options) {
        ...
        return BufferConverter.fromData64Url(data64Url)
            .then(function(buffer) {
                options.mime = options.mime || buffer.mime;
                return _this.addAssetFromBuffer(buffer.data, options);
            })
    ...
    addAssetFromBlob: function(blob, options) {
        ...
        return BufferConverter.fromBlob(blob)
            .then(function(buffer) {
                options.mime = options.mime || buffer.mime;
                return _this.addAssetFromBuffer(buffer.data, options);
            }).then(function(id) {
                _this.$data.assetsList[id].as.blob = blob;
                return id;
            });
    },
    ...
    getAssetAsBlob: function(id) {
        ...
        if (!asset.as.blob) {
            return BufferConverter.toBlob(asset.as.buffer)
                .then(function(blob) {
                    asset.as.blob = blob;
