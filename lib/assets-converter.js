...
var AssetsConverter = Class.$extend({
    ...
    __classvars__: {
        ...
        _blobUrlToBuffer: function(asset) {
            return httpRequest._operations._getProxy(asset.as.blobUrl)
                .then(httpRequest._operations._checkHeaders)
                .then(httpRequest._operations._readBody)
                .then(function(response) {
                    asset.as.buffer = response.body;
                    asset.mime = asset.mime || response.headers["content-type"].split(";")[0].toLowerCase();
                    return Q(asset);
                });
        },
